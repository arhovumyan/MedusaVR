...
# Create proper directory structure (works on both local and server)
if [[ "$USER" == "root" ]] || [[ -w "/root" ]]; then
...
echo "Starting character generation for: $CHARACTER_NAME" > "$GENERATION_LOG" 2>/dev/null || true
echo "Timestamp: $TIMESTAMP" >> "$GENERATION_LOG" 2>/dev/null || true
echo "Description: $CHARACTER_DESC" >> "$GENERATION_LOG" 2>/dev/null || true
echo "----------------------------------------" >> "$GENERATION_LOG" 2>/dev/null || true

# Function to check job completion using prompt_id
check_job_completion() {
    local prompt_id=$1
    local max_wait=30  # Reduced since we know it takes ~7 seconds
    local wait_time=0
    
    echo "⏳ Waiting for generation (estimated 7 seconds)..."
    
    # Simple fixed wait since we know the timing
    sleep 8
    
    echo "✅ Generation should be completed!"
    return 0
}
...
download_by_prompt_pattern() {
    ...
    local cache_file="$CACHE_DIR/last_index"
    local base_num=$(cat "$cache_file" 2>/dev/null || echo 0)
    ...
    if curl -s --fail "$URL/view?filename=$filename" -o "$output_file" 2>/dev/null; then
        if [ -s "$output_file" ] && file "$output_file" 2>/dev/null | grep -q "PNG image"; then
            ...
            echo "SUCCESS: $filename -> $(basename "$output_file")" >> "$GENERATION_LOG" 2>/dev/null || true
            GENERATED_FILES+=("$output_file")
            echo $test_num > "$cache_file" 2>/dev/null || true
            return 0
        else
            rm -f "$output_file"
        fi
    ...
    echo "FAIL: Could not download for prompt_id $prompt_id" >> "$GENERATION_LOG" 2>/dev/null || true
    return 1
}
