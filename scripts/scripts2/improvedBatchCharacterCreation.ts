...
class ImprovedBatchCharacterCreation {
  ...
  async updateCharacterStats(characterId: number, likes: number, chats: number): Promise<void> {
    try {
      ...
      if (result.modifiedCount > 0) {
        ...
        console.log(` No character found with ID ${characterId} to update stats`);
      }
    } catch (error) {
  ...
  async processCharactersSequentially(characters: CharacterData[]): Promise<void> {
    ...
    console.log(` Each character takes ~4-5 minutes (avatar + embeddings), total estimated time: ${Math.round(characters.length * 5)} minutes`);
  ...
  async run(): Promise<void> {
    ...
    try {
      ...
      console.log(` Skipped (already exist): ${this.results.skipped} characters`);
