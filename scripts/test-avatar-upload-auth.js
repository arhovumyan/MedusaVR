...
async function testAvatarUploadWithAuth() {
  try {
    ...
    console.log('2. Testing avatar upload with authentication...');
    
    // Test the avatar upload endpoint with a mock JWT token
    // Note: This is a test token - in real usage, you'd get this from login
    const mockJwtToken = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VySWQiOiI2OGE2ODRkMmM2NWM0MTdkOTI1ZjVhMjciLCJpYXQiOjE3NTYyNjg2ODgsImV4cCI6MTc1NjI3MjI4OH0.test';
    ...
    if (uploadResponse.ok) {
      ...
      // If it's an authentication error, that's expected with a mock token
      if (uploadResponse.status === 401) {
        console.log('ℹ️ Expected authentication error with mock token');
        console.log('✅ CSRF protection is working correctly');
        console.log('✅ The upload endpoint is properly protected');
      }
    }
