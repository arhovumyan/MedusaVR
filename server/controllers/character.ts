...
export const listCharacters = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const { 
      ...
      offset = '0',
      excludeIds = '',
      mode = 'all', // 'all', 'featured', 'discover'
      page = '0',
      pageSize = '40'
    } = req.query;
    
    const shouldRandomize = randomize === 'true';
    const limitNum = Math.min(parseInt(limit as string) || 50, 1000); // Max 1000 for image generation page
    const offsetNum = parseInt(offset as string) || 0;
    const excludeIdsList = excludeIds ? (excludeIds as string).split(',').map(id => parseInt(id)).filter(id => !isNaN(id)) : [];
    const pageNum = parseInt(page as string) || 0;
    const pageSizeNum = Math.min(parseInt(pageSize as string) || 40, 1000); // Max 1000 per page
    ...
    if (!shouldRandomize && excludeIdsList.length === 0) {
      try {
        const cachedCharacters = await cacheService.getCharacterList(mode as string, pageNum, authUserId);
    ...
    if (!shouldRandomize && excludeIdsList.length === 0 && transformedChars.length > 0) {
      cacheService.cacheCharacterList(transformedChars, mode as string, pageNum, authUserId)
...
export const updateCharacterTags = async (characterId: string, newSelectedTags: Record<string, string[]>, oldSelectedTags?: Record<string, string[]>) => {
  try {
    ...
    let currentTagIds: mongoose.Types.ObjectId[] = [];
...
export const likeCharacter = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const characterId = req.params.id;
...
export const unlikeCharacter = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const characterId = req.params.id;
    const numericId = parseInt(characterId, 10);
...
export const getCharactersByTags = async (req: Request, res: Response, next: NextFunction) => {
  try {
 ...
 */
export async function regenerateCharacterImage(req: Request, res: Response) {
  try {
    ...
    const imageResult = await CharacterImageService.generateCharacterConsistentImage(
      characterId,
      character.avatar, // Use current character image as base
      newPrompt,
      {
        name: character.name,
