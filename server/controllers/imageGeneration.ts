...
import { PlaceholderImageService } from '../services/PlaceholderImageService.js';
...
export class ImageGenerationController {
  ...
  private cloudinaryService: CloudinaryFolderService;
  ...
  generateImage = async (req: Request, res: Response): Promise<void> => {
    try {
      const {
        prompt,
        style = 'anime',
        artStyle, // New parameter from frontend for character consistency
        width = 1024,
        height = 1536,
        characterId,
        characterName,
      ...
      // Define coin costs for different quantities
      const coinCosts: { [key: number]: number } = {
        1: 6,
        2: 12,
        4: 24,
        8: 35,
      ...
      // Test bypass: allow injecting a fake user (used by lightweight tests)
      const user = (process.env.__TEST_FAKE_USER__ && (global as any).__FAKE_USER__)
        ? (global as any).__FAKE_USER__
      ...
      console.log(`🚀 Starting async image generation for user ${userId} with ${quantity} image(s). Deducted ${requiredCoins} coins. Remaining coins: ${user.coins}`);
   ...
   */
  getUserJobs = async (req: Request, res: Response): Promise<void> => {
    try {
      const userId = req.user?.id;
  ...
  getModels = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      res.json({
        success: true,
        data: {
          models: availableModels.map((model: string) => ({
            id: model,
          ...
          availableSamplers: availableSamplers.map((sampler: string) => ({
            id: sampler,
            name: sampler,
   ...
   */
  checkEmbeddingAvailability = async (req: Request, res: Response): Promise<void> => {
    try {
   ...
   */
  getCharacterImages = async (req: Request, res: Response): Promise<void> => {
    try {
      const { characterId } = req.params;
   ...
   */
  generateImageImmediate = async (req: Request, res: Response): Promise<void> => {
    try {
      const {
        prompt,
        characterId,
      ...
      console.log(`🚀 Starting immediate image generation for user ${userId} with ${quantity} image(s). Deducted ${requiredCoins} coins. Remaining coins: ${user.coins}`);
      ...
      const result = await this.embeddingService.generateImageWithEmbedding({
        ...
        immediateResponse: true, // This is the key optimization
        useGothicLora,
        loraStrength,
        loraModel
      });
      ...
      if (!result.success) {
        // Refund coins if generation failed
        user.coins += requiredCoins;
        await user.save();
      ...
      // Return immediate RunPod URLs
      res.json({
        success: true,
        data: {
          imageUrls: result.imageUrls || [],
          imageUrl: result.imageUrl, // For backward compatibility
