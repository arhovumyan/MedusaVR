...
export const avatarUpload = (req: any, res: any, next: any) => {
...
export async function uploadAvatar(req: Request, res: Response) {
  try {
    ...
    const updatedUser = await UserModel.findByIdAndUpdate(
      userId,
      { avatarUrl: avatarUrl }, // Use avatarUrl field
      { new: true }
    ).select("-password");
...
export async function uploadCharacterImage(req: Request, res: Response) {
  try {
    const userId = (req as any).userId; // Set by auth middleware
    const file = req.file;
...
export async function uploadAvatarBase64(req: Request, res: Response) {
  try {
    const userId = (req as any).userId; // Set by auth middleware
