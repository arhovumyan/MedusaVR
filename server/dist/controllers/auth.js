...
export async function refreshToken(req, res) {
    try {
        ...
        const newAccessToken = jwt.sign({ userId: user._id.toString() }, JWT_SECRET, { expiresIn: JWT_EXPIRES_IN });
...
export async function updateProfile(req, res) {
    try {
        ...
        // Update user
        const updatedUser = await UserModel.findByIdAndUpdate(userId, updateData, { new: true, runValidators: true }).select("-password");
        if (!updatedUser) {
            return res.status(404).json({ message: "User not found" });
        }
        res.json({
...
export async function addCoins(req, res) {
    try {
        ...
        // Update user coins
        const updatedUser = await UserModel.findByIdAndUpdate(userId, { $inc: { coins: amount } }, { new: true, runValidators: true }).select("-password");
        if (!updatedUser) {
            return res.status(404).json({ message: "User not found" });
        }
        res.json({
