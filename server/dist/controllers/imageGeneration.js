...
export class ImageGenerationController {
    constructor() {
        ...
        this.generateImage = async (req, res) => {
            try {
                const { prompt, style = 'anime', artStyle, // New parameter from frontend for character consistency
                ...
                loras, enableHr, hrUpscaler, hrScale, denoisingStrength, negativePrompt, 
                // Accept new LoRA fields from chat modal
                loraModel, loraStrength } = req.body;
                ...
                const user = (process.env.__TEST_FAKE_USER__ && global.__FAKE_USER__)
                    ? global.__FAKE_USER__
        ...
        this.generateImageImmediate = async (req, res) => {
            try {
                const { prompt, characterId, width = 1024, height = 1536, steps, cfgScale, seed, quantity = 1, negativePrompt, useGothicLora = false, loraStrength = 0.8, loraModel } = req.body;
                // Define coin costs for different quantities
                const coinCosts = {
                    1: 6,
                    2: 12,
