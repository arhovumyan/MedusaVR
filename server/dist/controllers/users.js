...
export async function updateUser(req, res) {
    try {
        ...
        const user = await UserModel.findByIdAndUpdate(userId, filteredUpdates, { new: true, runValidators: true }).select("-password");
        if (!user) {
            return res.status(404).json({ message: "User not found" });
        }
        res.json({
...
export async function updateUserPreferences(req, res) {
    try {
        ...
        const user = await UserModel.findByIdAndUpdate(userId, { $set: updateData }, { new: true, runValidators: true }).select('-password');
        if (!user) {
...
export async function getLikedCharacters(req, res) {
    try {
...
export async function unlikeCharacter(req, res) {
    try {
        const userId = req.params.id;
        const { characterId } = req.body;
        if (!characterId) {
