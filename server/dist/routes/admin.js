 ...
 */
router.get('/users/:userId/violations', requireAuth, requireAdmin, async (req, res) => {
    try {
        ...
        res.json({
            ...
            summary: {
                ...
                recentViolations: violations.filter(v => new Date(v.timestamp).getTime() > Date.now() - (30 * 24 * 60 * 60 * 1000)).length
            }
        });
    }
    catch (error) {
 ...
 */
router.get('/users/:userId/compliance-report', requireAuth, requireAdmin, async (req, res) => {
    try {
        const { userId } = req.params;
...
router.get('/dashboard', requireAuth, requireAdmin, async (req, res) => {
    try {
        ...
        const [totalViolations, violations24h, violations7d, violations30d, ageViolations30d, bannedUsers, suspendedUsers, recentViolations] = await Promise.all([
...
router.get('/users', requireAuth, requireAdmin, async (req, res) => {
    try {
        ...
        const usersWithViolations = await Promise.all(users.map(async (user) => {
 ...
 */
router.delete('/users/:userId/delete-account', requireAuth, requireAdmin, async (req, res) => {
    try {
        const { userId } = req.params;
 ...
 */
router.post('/users/batch-delete-accounts', requireAuth, requireAdmin, async (req, res) => {
    try {
 ...
 */
router.get('/users/permanently-banned', requireAuth, requireAdmin, async (req, res) => {
    try {
        ...
        const bannedUsersWithViolations = await Promise.all(permanentlyBannedUsers.map(async (user) => {
            const violationCount = await ViolationModel.countDocuments({ userId: user._id });
