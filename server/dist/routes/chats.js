...
import { createPersonalitySystemMessage, getPersonalityVariation } from "../utils/personalityPrompts.js";
...
router.get("/user/:userId", requireAuth, async (req, res) => {
    try {
        ...
        const enrichedChats = await Promise.all(chats.map(async (chat) => {
            ...
            if (!character)
                return null; // Skip if character was deleted
            return {
                ...chat.toObject(),
                character: {
                    id: character.id,
...
export const generateAIResponse = async (message, characterName, characterPersona) => {
    try {
        console.log(`🤖 Generating AI response for ${characterName}...`);
        // Use Replicate service to generate the response
        const response = await replicateService.generateResponse(message, characterName, characterPersona);
        return response;
    }
    catch (error) {
    ...
    try {
        ...
        if (!chat) {
          ...
          top_p: 0.9
            ...
            await CharacterModel.findOneAndUpdate({ id: parseInt(characterId) }, { $inc: { chatCount: 1 } });
        }
        res.json({
            chatId: chat._id,
...
router.post("/:characterId/message", requireAuth, ContentModerationService.moderateChatMessage, async (req, res) => {
    try {
        ...
        const tagNames = character.selectedTags
            ? Object.values(character.selectedTags)
                .flat()
        ...
        }, username, process.env.AI_BEHAVIOR_PROMPT);
...
router.post('/generate', requireAuth, ContentModerationService.moderateChatMessage, async (req, res) => {
