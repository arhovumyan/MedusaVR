...
router.post('/upload-tester-png', async (req, res) => {
    try {
        ...
        if (isExistingCharacter) {
            uploadResult = await CloudinaryFolderService.uploadToPremadeCharacterFolder(imageBase64, TEST_USER_ID, testCharacter.name, 'tester_upload_test');
        }
        else {
            uploadResult = await CloudinaryFolderService.uploadToCharacterFolder(imageBase64, TEST_USER_ID, testCharacter.name, 'tester_upload_test');
        }
