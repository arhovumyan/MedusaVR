 ...
 */
router.get('/check-structure/:username/:characterName', async (req, res) => {
    try {
 ...
 */
router.post('/simulate-upload', async (req, res) => {
    try {
        ...
        const uploadResult = await BunnyFolderService.uploadPremadeCharacterImageWithIndexing(username, characterName, dummyImageBuffer);
        res.json({
            success: true,
 ...
 */
router.post('/reset-index', async (req, res) => {
    try {
        const { username, characterName } = req.body;
        if (!username || !characterName) {
            return res.status(400).json({
 ...
 */
router.get('/stats/:username/:characterName', async (req, res) => {
    try {
        const { username, characterName } = req.params;
