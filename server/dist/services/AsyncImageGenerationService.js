...
class AsyncImageGenerationService extends EventEmitter {
    constructor() {
        ...
        this.maxConcurrentJobs = 3; // Adjust based on server capacity
        ...
        try {
            ...
            if (request.characterId) {
                ...
                imageResult = await this.embeddingService.generateImageWithEmbedding({
                    ...
                    immediateResponse: request.immediateResponse || false, // Add immediate response option
                    // Add LoRA support
        ...
        try {
            ...
            if (quantity === 1) {
                ...
                if (result.success && result.imageUrl) {
                    ...
                    const uploadedUrl = await this.uploadImageToCloudinary(result.imageUrl, username, characterName, 1, request.characterId ? userId : undefined, // Pass userId only if we have a characterId
                    request.characterId);
                    if (uploadedUrl) {
            ...
            for (let index = 0; index < imageResults.length; index++) {
                ...
                if (result.status === 'fulfilled' && result.value.success && result.value.imageUrl) {
                    try {
                        ...
                        const uploadedUrl = await this.uploadImageToCloudinaryWithRetry(result.value.imageUrl, username, characterName, index + 1, request.characterId ? userId : undefined, // Pass userId only if we have a characterId
                        request.characterId, 3 // max retries
                        );
                        if (uploadedUrl) {
    ...
    async uploadImageToCloudinaryWithRetry(imageUrl, username, characterName, imageIndex, userId, characterId, maxRetries = 3) {
        ...
        for (let attempt = 1; attempt <= maxRetries; attempt++) {
            try {
                ...
                const result = await this.uploadImageToCloudinary(imageUrl, username, characterName, imageIndex, userId, characterId);
                if (result) {
    ...
    async uploadImageToCloudinary(imageUrl, username, characterName, imageIndex, userId, characterId) {
        try {
            ...
            if (userId && characterId) {
                ...
                uploadResult = await BunnyFolderService.uploadPremadeCharacterImageWithIndexing(username, characterName, imageBuffer);
                ...
                uploadResult = await BunnyFolderService.uploadToCharacterFolder(username, sanitizedCharacterName, imageBuffer, filename, 'images');
                if (uploadResult.success) {
