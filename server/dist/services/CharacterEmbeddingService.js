...
export class CharacterEmbeddingService {
    ...
    async generateEmbeddingImages(options) {
        ...
        try {
                        ...
                        try {
                            ...
                            if (imageResponse.ok) {
                                ...
                                if (imageBuffer.length > 0) {
                                    ...
                                    const uploadResult = await BunnyFolderService.uploadToCharacterFolder(options.username, options.characterName, imageBuffer, fileName, 'embeddings');
            ...
            const embeddingMetadataResult = await BunnyFolderService.uploadToCharacterFolder(options.username, options.characterName, Buffer.from(JSON.stringify(embeddingData, null, 2), 'utf-8'), `embedding-metadata-${Date.now()}.json`, 'embeddings');
    ...
    buildConsistentCharacterDescription(options) {
        ...
        if (options.selectedTags?.appearance) {
            const appearanceTags = options.selectedTags.appearance.map(tag => tag.replace('-', ' ').replace('_', ' '));
            parts.push(...appearanceTags);
        }
        // Add character type for context
        if (options.selectedTags?.['character-type']) {
            const characterTypes = options.selectedTags['character-type'].filter(type => !['sfw', 'nsfw'].includes(type));
    ...
    extractCharacterFeatures(basePrompt) {
        ...
        if (prompt.includes('dress'))
            features.push('wearing dress');
        if (prompt.includes('uniform'))
            features.push('wearing uniform');
        if (prompt.includes('casual'))
            features.push('casual clothing');
        if (prompt.includes('formal'))
            features.push('formal attire');
