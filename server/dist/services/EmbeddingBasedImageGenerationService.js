...
export class EmbeddingBasedImageGenerationService {
     ...
     */
    async generateSingleImage(character, embeddingPrompt, options, imageIndex, username, // Pass username directly to avoid duplicate lookups
    returnImmediateUrl = false, // New option to return RunPod URL immediately
    fileBasedIndex // Index from the file-based system
    ) {
        try {
            ...
            while (!queueComplete && pollAttempts < maxPollAttempts) {
                try {
                    ...
                    if (queueResponse.ok) {
                        ...
                        const isInQueue = queueData.queue_pending?.some((item) => item[1] === promptId) ||
     ...
     */
    async handleImmediateImageResponse(character, baseFilename, imageIndex, username, fileBasedIndex, // Index from the file-based system
    runpodUrl // URL for the specific art style
    ) {
        ...
        setImmediate(async () => {
            ...
            while (attempt < maxRetries) {
                try {
                    ...
                    const uploadResult = await BunnyFolderService.uploadToPremadeCharacterFolder(uploadData.username, sanitizedCharacterName, // Use sanitized name for consistency
                    imageBuffer, sequentialFilename, // Use file-based sequential filename to prevent overwrites
                    'images');
    ...
    async processImageWithFullDownload(character, baseFilename, imageIndex, username, fileBasedIndex, // Index from the file-based system
    runpodUrl // URL for the specific art style
    ) {
        ...
        try {
            ...
            const uploadResult = await BunnyFolderService.uploadToPremadeCharacterFolder(username, sanitizedCharacterName, // Use sanitized name for consistency
            latestImage.buffer, sequentialFilename, // Use file-based sequential filename to prevent overwrites
            'images');
            if (uploadResult.success) {
    ...
    buildNegativePrompt(character, userNegativePrompt) {
        ...
        if (character.imageGeneration?.negativePrompt) {
            ...
            negParts.push('lowres', 'bad anatomy', 'bad hands', 'text', 'error', 'missing fingers', 'extra digit', 'fewer digits', 'cropped', 'worst quality', 'low quality', 'normal quality', 'jpeg artifacts', 'signature', 'watermark', 'username', 'blurry', 'ugly', 'duplicate', 'morbid', 'mutilated', 'out of frame', 'extra fingers', 'mutated hands', 'poorly drawn hands', 'poorly drawn face', 'mutation', 'deformed', 'bad proportions', 'malformed limbs', 'extra limbs', 'cloned face', 'disfigured', 'gross proportions', 'missing arms', 'missing legs', 'extra arms', 'extra legs', 'fused fingers', 'too many fingers', 'long neck', 'oversaturated', 'bad composition', 'bad lighting', 'pixelated', 'low detail');
        }
