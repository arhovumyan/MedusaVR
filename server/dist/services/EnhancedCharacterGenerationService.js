...
class EnhancedCharacterGenerationService {
     ...
     */
    async createCharacterFolders(characterName, username) {
        try {
     ...
     */
    async uploadImageToCloudinary(imageUrl, folderPath, fileName) {
        try {
     ...
     */
    async createCharacterEmbeddings(images, seeds, characterData) {
        try {
            ...
            const embedding = Array.from({ length: 512 }, (_, i) => parseInt(hash.slice(i % hash.length, (i % hash.length) + 2), 16) / 255);
            return {
                text: embeddingText,
                vector: embedding,
                dimension: 512,
     ...
     */
    async uploadEmbeddings(embeddings, folderPath, characterName) {
        try {
            const embeddingsJson = JSON.stringify(embeddings, null, 2);
            const safeName = characterName.toLowerCase().replace(/[^a-z0-9]/g, '_');
            const result = await cloudinary.uploader.upload(`data:application/json;base64,${Buffer.from(embeddingsJson).toString('base64')}`, {
     ...
     */
    async generateCompleteCharacter(request) {
        ...
        try {
            ...
            const avatarUpload = await this.uploadImageToCloudinary(avatarResult.imageUrl, `${folderResult.folderPath}/avatar`, `${safeName}_avatar`);
            ...
            if (request.generateEmbeddings !== false) {
                ...
                if (embeddingResult.success && embeddingResult.images.length > 0) {
                    ...
                    for (let i = 0; i < embeddingResult.images.length; i++) {
                        ...
                        const uploadResult = await this.uploadImageToCloudinary(imageUrl, `${folderResult.folderPath}/embeddings`, `${safeName}_embed_${i + 1}`);
                        if (uploadResult.success) {
                            embeddingImageUrls.push(uploadResult.cloudinaryUrl);
                        }
                    }
                    // Create embeddings
                    embeddingData = await this.createCharacterEmbeddings(embeddingImageUrls, embeddingResult.seeds, request);
