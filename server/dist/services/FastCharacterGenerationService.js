...
export class FastCharacterGenerationService {
    ...
    async generateCharacterFast(options) {
        ...
        try {
            ...
            const { positivePrompt, negativePrompt } = await this.loadPrompts(options.positivePromptFile, options.negativePromptFile);
            ...
            const uploadResult = await BunnyFolderService.uploadToCharacterFolder(options.username, options.characterName, imageBuffer, avatarFileName, 'avatar');
            ...
            const embedding = Array.from({ length: 384 }, (_, i) => parseInt(hash.slice(i % hash.length, (i % hash.length) + 2), 16) / 255);
            const embeddings = {
                text: embeddingText,
                vector: embedding,
                dimension: 384,
            ...
            const embeddingsResult = await BunnyFolderService.uploadToCharacterFolder(options.username, options.characterName, Buffer.from(JSON.stringify(embeddings, null, 2), 'utf-8'), `embeddings-${Date.now()}.json`, 'embeddings');
     ...
     */
    async generateBatchCharactersFast(characterTemplates, batchSize = 3, delayBetween = 2000) {
        const results = [];
