...
export class ImageIndexService {
    ...
    static async updateImageIndex(username, sanitizedCharacterName, newIndex) {
        try {
            ...
            const uploadResult = await BunnyStorageService.uploadFile(indexFilePath, Buffer.from(indexContent, 'utf-8'), 'text/plain');
    ...
    static async ensurePremadeCharacterFolderStructure(username, characterName) {
        try {
            ...
            if (!fileResult.success || !fileResult.data) {
                ...
                const uploadResult = await BunnyStorageService.uploadFile(indexFilePath, Buffer.from('0', 'utf-8'), 'text/plain');
                if (uploadResult.success) {
