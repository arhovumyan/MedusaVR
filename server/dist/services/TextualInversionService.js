...
export class TextualInversionService {
    ...
    async trainTextualInversionEmbedding(options) {
        try {
            ...
            const uploadResult = await BunnyFolderService.uploadToCharacterFolder(options.username, options.characterName, embeddingBuffer, embeddingFileName, 'embeddings');
    ...
    async updateCharacterWithEmbedding(characterId, embeddingInfo) {
        try {
            await CharacterModel.findOneAndUpdate({ id: parseInt(characterId) }, {
                $set: {
