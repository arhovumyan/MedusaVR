 ...
 */
export function generateSecureTokens(userId: string, options?: {
 ...
 */
export function trackSession(sessionId: string, userId: string, req: Request) {
 ...
 */
export function removeSessionTracking(sessionId: string) {
 ...
 */
export const sessionRotation = async (req: Request, res: Response, next: NextFunction) => {
  ...
  if (authHeader && authHeader.startsWith('Bearer ')) {
    try {
      ...
      if (timeToExpiry < 300 && decoded.type === 'access') { // Less than 5 minutes
        ...
        // Generate new tokens
        const { accessToken, refreshToken, sessionId, expiresAt } = generateSecureTokens(
          decoded.userId, 
 ...
 */
export const secureLogout = (sessionId?: string) => {
  if (sessionId) {
 ...
 */
export const validateSessionSecurity = async (req: Request, res: Response, next: NextFunction) => {
  const authHeader = req.headers.authorization;
