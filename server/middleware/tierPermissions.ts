    ...
    try {
      ...
      if (userTierIndex < requiredTierIndex) {
        return res.status(403).json({ 
          ...
          availablePlans: Object.keys(SUBSCRIPTION_PLANS).filter(plan => 
            tierHierarchy.indexOf(plan) >= requiredTierIndex
          )
        });
      }
      ...
      (req as any).tierPermissions = TIER_PERMISSIONS[userTier];
...
export const checkPermission = (permission: keyof TierPermissions) => {
  return async (req: Request, res: Response, next: NextFunction) => {
    try {
      const userId = (req as any).userId;
      if (!userId) {
      ...
      // Add tier info to request for use in controllers
      (req as any).userTier = userTier;
      (req as any).tierPermissions = permissions;
