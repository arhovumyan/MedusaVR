...
import { 
  trainCharacterEmbedding, 
  getCharacterEmbeddingStatus,
  listCharacterEmbeddings
...
router
  ...
  .get("/auth-test", requireAuth, (req: any, res) => {
    res.json({ 
      success: true, 
  ...
  .post("/:id/generate-image", requireAuth, checkPermission('canCreateCharacters'), ImageModerationService.moderateImageGeneration, async (req: any, res) => {
    try {
