...
import { DeepgramVoiceService, VoiceCallOptions } from '../services/DeepgramVoiceService.js';
...
const activeSessions = new Map<string, DeepgramVoiceService>();
 ...
 */
router.post('/start/:characterId', requireAuth, async (req: AuthenticatedRequest, res: Response) => {
  try {
 ...
 */
router.post('/end/:characterId', requireAuth, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const { characterId } = req.params;
 ...
 */
router.get('/status/:characterId', requireAuth, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const { characterId } = req.params;
    const userId = req.userId; // Use req.userId
 ...
 */
export function handleVoiceAudio(sessionKey: string, audioData: Buffer): void {
  const voiceService = activeSessions.get(sessionKey);
 ...
 */
export function handleVoiceSessionCleanup(sessionKey: string, reason: string): void {
  try {
    const voiceService = activeSessions.get(sessionKey);
 ...
 */
export async function cleanupVoiceSessions(): Promise<void> {
