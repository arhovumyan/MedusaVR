...
async function setupBatchUser() {
  try {
    ...
    let batchUser = await UserModel.findOne({ email: 'batch@medusavr.com' });
...
async function createCharacter(template: CharacterTemplate, userId: string): Promise<any> {
  try {
    ...
    if (existingCharacter) {
      console.log(`⚠️ Character "${template.name}" already exists, skipping...`);
    ...
    if (result.success && result.character) {
      ...
      if (embeddingResult.success) {
        ...
        console.log(`⚠️ Embedding generation failed for ${template.name}: ${embeddingResult.error}`);
      }
...
async function main() {
  ...
  for (let i = 0; i < characterCount; i++) {
    ...
    do {
      ...
      attempts++;
      if (attempts > 50) {
        console.warn(`⚠️ Could not generate unique name after 50 attempts, using: ${template.name}`);
        break;
      }
