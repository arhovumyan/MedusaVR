...
export class BunnyFolderService {
   ...
   */
  static async uploadPremadeCharacterImageWithIndexing(
    username: string, 
    characterName: string, 
    fileBuffer: Buffer | string
  ): Promise<{success: boolean, url?: string, publicId?: string, fileName?: string, imageNumber?: number, error?: string}> {
    try {
  ...
  static async batchUploadToPremadeCharacterFolder(
    ...
    concurrency: number = 2, // Reduced default concurrency for better reliability
    ...
    for (let i = 0; i < skippedCount; i++) {
      const skippedUpload = uploads.find(u => 
        !filteredUploads.some(fu => fu.fileName === u.fileName)
      );
      if (skippedUpload) {
        results.push({
          success: true,
