...
export class BunnyStorageService {
   ...
   */
  static async downloadFile(filePath: string): Promise<{success: boolean, data?: Buffer, error?: string}> {
    try {
      this.ensureConfigured();
   ...
   */
  static async listFiles(directoryPath: string): Promise<{success: boolean, files?: string[], error?: string}> {
    try {
      this.ensureConfigured();
      ...
      const files = Array.isArray(data) ? data.map((item: any) => item.ObjectName).filter((name: string) => name && !name.endsWith('/')) : [];
   ...
   */
  static async createDirectory(directoryPath: string): Promise<{success: boolean, error?: string}> {
    try {
