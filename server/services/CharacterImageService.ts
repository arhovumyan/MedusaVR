...
export class CharacterImageService {
   ...
   */
  static async generateCharacterAvatar(
    characterData: {
      name: string;
      description: string;
    ...
    userId: string,
    username: string
  ): Promise<{ success: boolean; imageUrl?: string; error?: string; generationData?: any }> {
  ...
  static async generateCharacterVariation(
    ...
    try {
      ...
      const variationResult = await CharacterGenerationService.generateCharacterVariation(
        characterSeed, 
        variationType, 
        generationOptions
      );
   ...
   */
  private static async uploadUrlToBunny(
    imageUrl: string,
    username: string,
   ...
   */
  private static async uploadToBunny(
    imageBuffer: Buffer,
    username: string,
    characterName: string
  ): Promise<{
   ...
   */
  static generatePlaceholderImage(characterName: string, artStyle?: string): string {
   ...
   */
  static async checkGenerationViability(): Promise<{
    viable: boolean;
    reason?: string;
  }> {
   ...
   */
  static async generateAdditionalImage(
    characterId: number,
   ...
   */
  static async generateCharacterConsistentImage(
    characterId: string,
    baseImageUrl: string, // Existing character image URL
    newPrompt: string,
    characterData: {
      name: string;
      description: string;
    ...
    userId: string,
    username: string,
    denoisingStrength: number = 0.2 // Lower = more similar to original
  ): Promise<{ success: boolean; imageUrl?: string; error?: string; generationData?: any }> {
    try {
