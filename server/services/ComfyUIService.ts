...
class ComfyUIService {
   ...
   */
  private createCharacterWorkflow(request: ComfyUIRequest): any {
   ...
   */
  async submitWorkflow(workflow: any): Promise<{ success: boolean; promptId?: string; error?: string }> {
    try {
   ...
   */
  async waitForCompletion(promptId: string, characterName: string, maxWaitTime: number = 120000): Promise<{ success: boolean; imageUrl?: string; error?: string }> {
   ...
   */
  private async findLatestImage(characterName: string): Promise<string | null> {
   ...
   */
  async generateCharacterImage(request: ComfyUIRequest): Promise<ComfyUIResponse> {
    try {
   ...
   */
  async generateEmbeddingImages(request: ComfyUIRequest, count: number = 5): Promise<{
    success: boolean;
    images: string[];
    seeds: number[];
    error?: string;
