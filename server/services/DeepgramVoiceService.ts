...
export class DeepgramVoiceService extends EventEmitter {
  ...
  private maxHistoryLength: number = 6; // Keep last 6 messages (3 exchanges)
   ...
   */
  async startVoiceCall(options: VoiceCallOptions): Promise<void> {
    try {
      ...
      await new Promise((resolve, reject) => {
        ...
        setTimeout(() => {
          ...
          if (typeof (live as any).setupConnection === 'function') {
            console.log('🚀 Found setupConnection method, calling it...');
            try {
              (live as any).setupConnection();
   ...
   */
  private setupTranscriptionListeners(options: VoiceCallOptions): void {
    ...
    this.liveTranscription.on(LiveTranscriptionEvents.SpeechStarted, (data: any) => {
   ...
   */
  private async processUserSpeech(transcript: string, options: VoiceCallOptions): Promise<void> {
    try {
   ...
   */
  private buildTagBasedPersonality(tagNames: string[]): string {
   ...
   * Generate AI response using the specified model
   */
  private async generateAIResponse(userMessage: string, options: VoiceCallOptions): Promise<string> {
    try {
...
Remember: ${options.username} is the person you're talking to.`;
   ...
   */
  private async textToSpeech(text: string, options: VoiceCallOptions): Promise<void> {
    try {
      ...
      if (audioData) {
        ...
        if (audioData.pipe && typeof audioData.on === 'function') {
          ...
          audioData.on('data', (chunk: Buffer) => {
            chunks.push(chunk);
          });
   ...
   */
  getConnectionStatus(): LiveConnectionState | string {
