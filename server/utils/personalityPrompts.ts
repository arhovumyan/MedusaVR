...
/**
 * Personality-based prompt generation for character chat systems
 * Creates unique system messages based on character tags and descriptions
 */
 ...
 */
export function generateTagCombinationBehaviors(
  ...
  const matchingCombinations = combinations.filter(combo => 
    combo.traits.every(trait => allTraits.includes(trait))
  );
 ...
 */
export function generatePersonalityPrompts(
  personalityTraits?: { mainTrait?: string; subTraits?: string[] },
  tags: string[] = [],
  isNSFW: boolean = false,
  description?: string
 ...
 */
export function extractPersonalityFromDescription(description: string): string[] {
...
export function generateNSFWBehaviorPrompt(): string {
 ...
 */
export function createPersonalitySystemMessage(
 ...
 */
export function getPersonalityVariation(tags: string[]): string {
  const variations = [
